...
| DEC-20260831-015 | Implemented locally; platform evidence pending | All authenticated roles use the full installed application catalog (`only_owner=false`); application owner does not narrow catalog use, while control data and backup destinations remain session-scoped | REQ-20260830-008 |
| DEC-20260831-014 | Adopted locally; official per-instance data API evidence pending | appvar projection availability and access errors, including LightOS-isolated data, are reported as system/permission restricted without an app-ID allowlist or privileged fallback | REQ-20260831-011 |
| DEC-20260901-016 | Implemented locally; `user.notify`, device message delivery and browser permissions pending | Task terminal results use Lazycat SDK `MessageService.NewMessage` and, after explicit browser consent, Web Notifications; failures are non-fatal and current-tenant scoped | REQ-20260901-012 |
