#!/bin/sh
set -eu

REPO_ROOT=$(CDPATH= cd -- "$(dirname -- "$0")/.." && pwd)
DIST_ROOT="$REPO_ROOT/lzc-dist"

rm -rf "$DIST_ROOT"
mkdir -p "$DIST_ROOT/bin" "$DIST_ROOT/web" "$DIST_ROOT/lzc"

cd "$REPO_ROOT"
npm ci --prefix apps/web
npm run api:generate --prefix apps/web
(cd apps/web && npx vite build)

CGO_ENABLED=0 GOOS=linux GOARCH=amd64 \
  go build -trimpath -ldflags='-s -w' \
  -o "$DIST_ROOT/bin/backup-server" ./apps/server/cmd/server

cp -a "$REPO_ROOT/apps/web/dist/." "$DIST_ROOT/web/"
cp "$REPO_ROOT/lzc/run.sh" "$DIST_ROOT/lzc/run.sh"
chmod 0755 "$DIST_ROOT/bin/backup-server" "$DIST_ROOT/lzc/run.sh"

find "$DIST_ROOT" -maxdepth 3 -type f -print | sort
